...
nestData <- read_csv("nest_data/nest_data_cleaned_0915_0008_.csv",
...
* Column for nest misclassification based on hatch or fail, not multiple
failure causes.

Where does unknown fate fit into this? In one paper, they had a third category, but could it just be misclassified?
...
* Column for nest misclassification including all fate categories.

* Column for how the nest was misclassified (correct, newly assigned from
unknown, incorrect)
...
ndGLM <- ndGLM |> mutate(is_u = isU(final_fate))
...
* Ungroup dataframe and make categorical variables into factors; make sure other data classes are correct
```{r}
...
* Examine and remove missing data (NA)
...
##### Frequency tables for vars in nest data, including 2021

```{r echo=FALSE, message=FALSE}
# can add labels to the variables in the table, or beforehand via attr from labelled package

# cat("Categorical predictor variables:\n")
# pre <- ndGLM_ |> tbV3jl_summary(include=c(species, cam_fate),
#                              label=list(species ~ "Species",
#                                         cam_fate ~ "True fate (from camera)"))
# pre
# pFr <- ndGLM_ |>
#   mutate(HF_mis   = HF_mis == 1,
#          misclass = misclass == 1,
#          is_u     = is_u == 1) |>
#   select(HF_mis, misclass, is_u, species, cam_fate) |>
#   tbl_strata(
#     strata = 
#   )
...
pr <- pr |> as_gt() |> 
  ...
  # cols_width(var_label ~ px(250),
             # everything() ~ px(100) ) #|>
  # cols_width(list(stub() ~ px(250),
             # everything() ~ px(100) )) #|>
  # cols_width(stub() ~ px(200))
#Error in `check_vars_dt_boxhead()`:! The `var` value must be value in `dt_boxhead$var`.
pr
now = format(Sys.time(), "%m%d_%H%M_")
# pr
...
pr |> gtsave(filename=filename, path="images/")
# requires webshot2 package
# pr
# why doesn't the merged table keep my spanning headers?

# weird, bc the documentation says for dichotomous vars, only "TRUE" or "1" is shown 
# but when it's 0/1 the table shows both, and when boolean it shows FALSE
  # this is not a gt object, so it prints using kable and does not have formatting (in pdf)
# actually, it IS a gt table, not sure why it doesn't print correctly
# there is a website bookmarked that explains I think?
...
# tbl_merge(tbls = list(pre, res),
          # tab_spanner = c("Categorical predictors", "Response"))
#################################################################################
...
# combined table, from stack exchange:
...
# but I just want to display side by side
# might have to save them and then print using cowplot?

cat("Categorical predictor variables:\n")
pre <- ndGLM_ |> tbl_summary(include=c(species, cam_fate),
                             label=list(species ~ "Species",
                                        cam_fate ~ "True fate (from camera)"))
pre
...
res <- ndGLM_ |> tbl_summary(include=c(is_u, HF_mis, misclass),
                             label=list(is_u ~ "Unknown in field",
                                        HF_mis ~ "Misclassified (H/F)",
                                        misclass ~ "Misclassified (all fates)")) 
# tbl_summary is not a gt object?

  # as_latex()

res
# knitr::kable(list(pre, res))
# need to add formatting first I think 
# or in kable?
# knitr::kable(list(pre, res), format="latex") # this displays nothing
# maybe would work in pdf output?
# in pdf, creates weird table
#################################################################################
...
Make sure the bin sizes make sense
...
I don't think the p-values are adding anything, since there is no correlation?
...
mod_diff <- function(mNull, mAlt){ # argument order matters - names should help
  ...
  # cat("\n---------------------------------------------------------------")
  cat("Percent difference in model coefficients (vs full model):\n\n")
...
Functions to create combo AIC/BIC tables
...
```{r}
# aic_tab <- function(nAIC){
#   
#   aTab <- nAIC |> select(c(Modnames, K_1, AICc_1, AICcWt_1, LL_2))
#   aTab <- aTab |> 
...
#### QUESTION 1: How do the predictor vars affect H/F classification accuracy?
