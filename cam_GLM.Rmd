...
nestData <- read_csv("nest_data/nest_data_cleaned_0913_2056_.csv",
...
ndGLM <- nestData %>% 
  ...
  select(nest, site, species, cov_5m, fate_date, nest_age, obs_int, i, j, k, final_fate, fate, cam_fate, status, year )%>%
...
ndGLM_ <- ndGLM_ |> mutate(
  ...
  across(c(species, cam_fate, HF_mis, misclass, how_mis, is_u), as.factor)
)
```
...
# obs <- table(ndGLM_$obs_int) |> as.data.frame(col.names=c("obs int", "freq")) #|> t() #|> as.numeric() |> print(row.names=FALSE)
# as.data.frame(c("obs int", "count"),table(ndGLM_$obs_int))
# as.data.frame(table(ndGLM_$obs_int), row.names=c("obs int", "count"))
# as.data.frame(table(ndGLM_$obs_int), row.names=c("obs int", "count"))

# count(ndGLM_, "obs_int")
# ndGLM_ %>% count(obs_int) # makes a df
# ndGLM_ %>% count(obs_int) %>% pivot_wider() knitr::kable()
# ndGLM_ %>% count(obs_int) %>% pivot_wider() knitr::kable()
# ndGLM_ %>% count(obs_int) %>% table()
# ndGLM_ %>% group_by(obs_int) |> count(obs_int) %>% tabulate() # tabulate isn't expecting a table as input, but a vector
# ndGLM_ %>% group_by(obs_int) %>% tabulate()
# table(ndGLM$obs_int, row.names=c("obs.int", "frequency"))
# table(ndGLM$obs_int, dnn=c("obs.int"))
# table(cat("obs int"),ndGLM$obs_int)
...
# cat("\nwas nest assigned a fate in the field?")
# table(ndGLM1$HF_mis)
...
# sapply(ndGLM1, function(x) sum(is.na(x)))
# sapply(ndGLM1, function(x) which(is.na(x))) # or do this for each nest 
...
# cat("\nwas nest assigned a fate in the field?")
# cat("\nwas nest fate unknown in the field?")
# table(ndGLM2$is_u)
