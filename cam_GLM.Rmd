...
fr_tab <- function(data){
  ...
  filename <- ifelse(
    "2021" %in% data$year,
    sprintf("images/frtab_inc2021_%s.png", now),
    sprintf("images/frtab_no2021_%s.png", now)
  )
  # filename <- sprintf("frtab_w2021_%s.png", now)
  # gtsave(pr, filename=sprintf("frtab_w2021_%s.png", now), path="images/")
  pr |> gtsave(filename=filename, path="analysis/")
...
# pr
# pr |> gtsave(filename=sprintf("images/frtab_w2021_%s.png", now))
filename <- sprintf("frtab_no2021_%s.png", now)
# gtsave(pr, filename=sprintf("frtab_w2021_%s.png", now), path="images/")
pr |> gtsave(filename=filename, path="analysis/")
...
knitr::include_graphics(paste0("analysis/", fn))
```
