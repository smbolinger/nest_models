...
ndGLM <- ndGLM %>% 
  mutate(c_hatchfail = case_when(
    ...
    cam_fate == 1           ~ 1,
    cam_fate == 7           ~ 7,  # hopefully not many/any of these
    # is.na(cam_fate)         ~ 7
    is.na(cam_fate)         ~ NA 
  ))
...
ndGLM <- ndGLM %>% 
  mutate(HF_mis = case_when(
    ...
    c_hatchfail == 7         ~ NA, # if cam fate=unknown, don't know misclass. or not
...
ndGLM1 <- ndGLM_ %>% select(!c(status)) # remove status (list of lists)
cat("All the missing values (with 2021):\n\n")
# sapply(ndGLM_, function(x) which(is.na(x))) # or do this by nest number
# sapply(ndGLM_, function(x) sum(is.na(x)))
ndGLM1 |> 
  select(!c(fate, i, j, k, c.fate, numNA, numNA1920))|>
...
ndGLM1 <- ndGLM1 |> 
  mutate(  across(c(species, cam_fate, c.fate, HF_mis, misclass, how_mis, is_u), as.factor)) # do this AFTER removing all the NAs so you don't get empty levels
...
cMat <- ndGLM1 |>
  select(nest_age, fdate, obs_int) |>
  cor() |>
  corrplot.mixed(order="AOE")
```
```{r}

cMat <- ndGLM2 |>                            # Exclude 2021
  select(nest_age, fdate, obs_int) |>
  cor() |>
  corrplot.mixed(order="AOE")
...
```{r eval=FALSE, include=FALSE}
ndGLM_scl1 <- read.csv("nest_data/scaled1.csv")
ndGLM_scl2 <- read.csv("nest_data/scaled2.csv")
```
...
```{r echo=FALSE, message=FALSE}
# aicHF <- aic_mod(mods[c(1, 4, 7:11)]) # mod 4 is main effects model
aicHF <- aic_mod(mods[c(1, 4, 7:length(mods))]) # mod 4 is main effects model
...
# mods[[13]] <- glm(as.formula(paste0(
#   
#   resp, "~ obs_int + cam_fate * nest_age"
#   
# )), data=dataMod, family=binomial) # this was supposed to combine the 2 best models
# # gives the "fitted probabilities numerically 0 or 1 occurred" warning 

# mods[[14]] <- glm(as.formula(paste0(
#   
#   resp, "~ obs_int * cam_fate + nest_age"
#   
# )), data=dataMod, family=binomial)
# 
# mods[[15]] <- glm(as.formula(paste0(
#   
#   resp, "~ obs_int * nest_age + cam_fate"
#   
# )), data=dataMod, family=binomial)
```
...
```{r echo=FALSE}
aicF <- aic_mod(mods[c(1,4,7:length(mods))])
...
```{r echo=FALSE}
aicf <- aic_mod(mods[c(1,5,7:length(mods))])
filename <- save_tab(aicf)
...
```{r echo=FALSE}
# oddRat <- odd_rat(mods)
aicTab <- aic_mod(mods[c(1,5,7:length(mods))])
# aicTab
