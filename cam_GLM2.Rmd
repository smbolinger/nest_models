...
aic_mod <- function(mods){
  ...
  str_replace(modEQ, p, r) # still doesn't work w/o spaces
  f <- r("species * obs_int")
  h <- r("species*obs_int") # r creates the proper replacement...
  # but it still isn't substituted in properly
  # not creating the proper replacement anymore... no words, just operators D:
  # still the same w/o the spaces - actually now it's working?
  
  p <- paste(m[,1], sep="|", collapse="|") # this grabs the correct strings
  p <-paste0("\\b", m[,1], "\\b", collapse="|")
  # creates one long string, but not a character vector
  # but that's how "colours" is in the example...
  p
  
  modEQ1 <- paste(sapply(mods,  
                        # function(x) str_extract(dp(x$call), "(?<=~\\s).+(?=,\\sf)")
                        function(x) str_extract(dp(x$call), "(?<=~\\s).+(?=\")")
                        ), sep = " ")
  
  # this works but the spaces are wonky:
  # # (turns out I don't need to delete them)
  modEQ1 <- gsub(" ", "", modEQ)
  modEQ1
  # modEQ <- gsub("\\*", "\\\\*", modEQ)
  # modEQ
  m <- str_match(modEQ1, "(\\w+)\\*(\\w+)" )
  
  p <-paste0("\\b", m[,1], "\\b", collapse="|")
  p <- gsub("\\*", "\\\\*", p)
  p
  str_replace(modEQ1, p, r) # still doesn't work w/o spaces
  str_replace(modEQ1, p, r )
  
  str_extract_all(modEQ, p)
  str_extract_all(x, colours) 
  # str_view(modEQ, "\\bspecies*obs_int\\b")
  str_view(modEQ, "species\\*obs_int") # so maybe I just need to properly escape the *?
  str_view(modEQ, "s")
  
  str_replace(modEQ, m, r )
  str_replace(modEQ, n[,1], r )
  # string recycling error, fml 
  
  # what if we took out all the spaces first, then added them back in if need be?
